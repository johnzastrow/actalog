...
- [x] `[HIGH]` **Admin: Create User flow on User Management screen (v1.3.1)** — `POST /api/admin/users` + "Create User" dialog. Decisions to make: initial password (admin-set vs invite-email vs reset-link), email verification policy, role picker scope, optional affiliation. Tracked: [#217](https://github.com/johnzastrow/actalog/issues/217) *(Completed v1.3.1)*
- [x] `[HIGH]` **Admin: edit full user details on Profile tab (v1.3.1)** — extend Profile tab with admin-set password (separate from existing force-password-reset), role change, account-disabled toggle. Brainstorm together with create-user since both touch admin password handling *(Completed v1.3.1)*
- [ ] `[HIGH]` **Admin: break-glass CLI for protected users (v1.3.1)** — `actalog admin force-edit-protected --email=X --field=name --value="..."` for documented operator escape hatch when a protected account needs operator-level edits. Admin screen + L1/L2 stay locked; this CLI is the only path. Requires explicit `--confirm` flag, audit log entry tagged `protected_user_break_glass`, post-edit re-run of boot invariant. Document in `docs/security/PROTECTED_USERS.md` §recovery
